...
internal enum class FailureKind(val description: String) {
    ...
    FLUSH("failed to flush"),
    PRIMARY_WRITE("primary write failed"),
    REPLAY("failed to replay recovered entries"),
    LOAD("failed to load stored entries"),
    REMOVE_UNREADABLE("failed to remove unreadable entries"),
    DECRYPT("failed to decrypt a preference"),
